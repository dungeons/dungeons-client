...
/**
 * <p>
 * Encapsulates OpenGL ES 2.0 frame buffer objects. This is a simple helper class which should cover most FBO uses. It will automatically create a texture
 * for the color attachment and a renderbuffer for the depth buffer. You can get a hold of the texture by
 * {@link QuadTextureFrameBuffer#getColorBufferTexture()}. This class will only work with OpenGL ES 2.0.
 * </p>
 * 
 * <p>
 * FrameBuffers are managed. In case of an OpenGL context loss, which only happens on Android when a user switches to another application or receives an
 ...
 * @author mzechner
 */
public class QuadTextureFrameBuffer implements Disposable {
    /** the frame buffers **/
    private final static Map<Application, List<QuadTextureFrameBuffer>> buffers = new HashMap<Application, List<QuadTextureFrameBuffer>>();
     ...
     */
    public QuadTextureFrameBuffer(Pixmap.Format format, int width, int height, boolean hasDepth) {
        this.width = width;
        this.height = height;
        this.format = format;
    ...
    private void addManagedFrameBuffer(Application app, QuadTextureFrameBuffer frameBuffer) {
        List<QuadTextureFrameBuffer> managedResources = buffers.get(app);
        if (managedResources == null) {
            managedResources = new ArrayList<QuadTextureFrameBuffer>();
        }
    ...
    /**
     * Invalidates all frame buffers. This can be used when the OpenGL context is lost to rebuild all managed frame buffers. This assumes that the texture
    ...
    public static void invalidateAllFrameBuffers(Application app) {
        ...
        List<QuadTextureFrameBuffer> bufferList = buffers.get(app);
        if (bufferList == null) {
            return;
        }
